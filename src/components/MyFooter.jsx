import React from "react";
import { CDBBtn, CDBIcon, CDBBox } from "cdbreact";

const MyFooter = () => {
  return (
    <>
      <footer className="shadow">
        <CDBBox
          display="flex"
          justifyContent="between"
          alignItems="center"
          className="mx-auto py-4 flex-wrap"
          style={{ width: "80%" }}
        >
          <CDBBox display="flex" alignItems="center">
            <a
              href="/"
              className="d-flex align-items-center justify-content-center p-0 text-dark"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 64 64"
                width="64px"
                height="64px"
                baseProfile="basic"
              >
                <radialGradient
                  id="S7t5BFrV6qv0IqiKl2YOra"
                  cx="32"
                  cy="809.39"
                  r="31.259"
                  gradientTransform="translate(0 -775.89)"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#9ceeff" />
                  <stop offset=".642" stop-color="#c5f5ff" />
                  <stop offset="1" stop-color="#dcf9ff" />
                </radialGradient>
                <path
                  fill="url(#S7t5BFrV6qv0IqiKl2YOra)"
                  d="M59.307,42H45v-8h14.302c1.895,0,3.594-1.419,3.693-3.312C63.101,28.67,61.495,27,59.5,27H45	v-5h14.828c2.089,0,3.955-1.527,4.153-3.607C64.209,16.008,62.339,14,60,14H45v-4h9.302c1.895,0,3.594-1.419,3.693-3.312	C58.101,4.67,56.495,3,54.5,3H9.698C7.803,3,6.104,4.419,6.005,6.312C5.899,8.33,7.505,10,9.5,10H19v4h-3H9H4.172	c-2.089,0-3.955,1.527-4.153,3.607C-0.209,19.992,1.661,22,4,22h5h7h3v5H4.698c-1.895,0-3.594,1.419-3.693,3.312	C0.899,32.33,2.505,34,4.5,34H19v6H4.693c-2.35,0-4.449,1.718-4.672,4.058C-0.235,46.742,1.869,49,4.5,49h10	c2.485,0,4.5,2.015,4.5,4.5v1c0,1.933-1.567,3.5-3.5,3.5h-3.389c-1.451,0-2.781,0.981-3.054,2.406C8.689,62.323,10.149,64,12,64	h39.889c1.451,0,2.781-0.981,3.054-2.406C55.311,59.677,53.851,58,52,58h-3.5c-1.933,0-3.5-1.567-3.5-3.5l0,0	c0-1.933,1.567-3.5,3.5-3.5h11c2.631,0,4.735-2.258,4.479-4.942C63.756,43.718,61.657,42,59.307,42z"
                />
                <linearGradient
                  id="S7t5BFrV6qv0IqiKl2YOrb"
                  x1="32"
                  x2="32"
                  y1="843.763"
                  y2="770.338"
                  gradientTransform="translate(0 -775.89)"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#005ce2" />
                  <stop offset=".278" stop-color="#0080e8" />
                  <stop offset=".569" stop-color="#009ced" />
                  <stop offset=".82" stop-color="#00b0f1" />
                  <stop offset="1" stop-color="#00b5f2" />
                </linearGradient>
                <path
                  fill="url(#S7t5BFrV6qv0IqiKl2YOrb)"
                  d="M18,8h28c5.523,0,10,4.477,10,10v28c0,5.523-4.477,10-10,10H18c-5.523,0-10-4.477-10-10V18	C8,12.477,12.477,8,18,8z"
                />
                <linearGradient
                  id="S7t5BFrV6qv0IqiKl2YOrc"
                  x1="46.998"
                  x2="46.998"
                  y1="836.78"
                  y2="777.976"
                  gradientTransform="translate(0 -775.89)"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#005ce2" />
                  <stop offset=".278" stop-color="#0080e8" />
                  <stop offset=".569" stop-color="#009ced" />
                  <stop offset=".82" stop-color="#00b0f1" />
                  <stop offset="1" stop-color="#00b5f2" />
                </linearGradient>
                <path
                  fill="url(#S7t5BFrV6qv0IqiKl2YOrc)"
                  d="M56,44v2c0,5.523-4.477,10-10,10h-5.5c-1.466,0-2.634-1.254-2.491-2.749	C38.134,51.946,39.329,51,40.64,51l6.86,0c0.83,0,1.5-0.67,1.5-1.5S48.33,48,47.5,48h0c-1.105,0-2-0.895-2-2v0c0-1.105,0.895-2,2-2	H56z"
                />
                <linearGradient
                  id="S7t5BFrV6qv0IqiKl2YOrd"
                  x1="15.999"
                  x2="15.999"
                  y1="847.425"
                  y2="783.144"
                  gradientTransform="translate(0 -775.89)"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#005ce2" />
                  <stop offset=".278" stop-color="#0080e8" />
                  <stop offset=".569" stop-color="#009ced" />
                  <stop offset=".82" stop-color="#00b0f1" />
                  <stop offset="1" stop-color="#00b5f2" />
                </linearGradient>
                <path
                  fill="url(#S7t5BFrV6qv0IqiKl2YOrd)"
                  d="M21,14h-5c-0.55,0-1.05,0.22-1.41,0.59C14.22,14.95,14,15.45,14,16c0,1.1,0.9,2,2,2h1	c1.21,0,2.18,1.08,1.97,2.34C18.81,21.32,17.89,22,16.89,22H8v-4c0-5.52,4.48-10,10-10h2.89c1.45,0,2.78,0.98,3.05,2.41	C24.31,12.32,22.85,14,21,14z"
                />
                <linearGradient
                  id="S7t5BFrV6qv0IqiKl2YOre"
                  x1="40.5"
                  x2="40.5"
                  y1="10.587"
                  y2="37.891"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#feaa53" />
                  <stop offset=".612" stop-color="#ffcd49" />
                  <stop offset="1" stop-color="#ffde44" />
                </linearGradient>
                <circle
                  cx="40.5"
                  cy="27.5"
                  r="8.5"
                  fill="url(#S7t5BFrV6qv0IqiKl2YOre)"
                />
                <linearGradient
                  id="S7t5BFrV6qv0IqiKl2YOrf"
                  x1="30"
                  x2="30"
                  y1="23"
                  y2="74.5"
                  gradientUnits="userSpaceOnUse"
                >
                  <stop offset="0" stop-color="#fff" />
                  <stop offset=".088" stop-color="#f8fdff" />
                  <stop offset=".216" stop-color="#e5f7fe" />
                  <stop offset=".369" stop-color="#c5eefc" />
                  <stop offset=".541" stop-color="#98e0fa" />
                  <stop offset=".728" stop-color="#60cff7" />
                  <stop offset=".926" stop-color="#1bbbf3" />
                  <stop offset="1" stop-color="#00b3f2" />
                </linearGradient>
                <path
                  fill="url(#S7t5BFrV6qv0IqiKl2YOrf)"
                  d="M38,29c-0.758,0-1.488,0.112-2.183,0.309C35.041,25.704,31.838,23,28,23c-4.418,0-8,3.582-8,8	c0,0.027,0.004,0.052,0.004,0.079C16.611,31.564,14,34.473,14,38c0,3.866,3.134,7,7,7h17c4.418,0,8-3.582,8-8S42.418,29,38,29z"
                />
              </svg>
            </a>
            <small className="ms-2">
              &copy; Giuliacci Dixit, 2023. All rights reserved.
            </small>
          </CDBBox>
          <CDBBox display="flex">
            <CDBBtn flat color="dark" className="p-2 ms-3">
              <CDBIcon fab icon="facebook-f" />
            </CDBBtn>
            <CDBBtn flat color="dark" className="mx-3 p-2">
              <CDBIcon fab icon="twitter" />
            </CDBBtn>
            <CDBBtn flat color="dark" className="p-2">
              <CDBIcon fab icon="instagram" />
            </CDBBtn>
            <CDBBtn flat color="dark" className="p-2 mx-3">
              <CDBIcon fab icon="youtube" />
            </CDBBtn>
            <CDBBtn flat color="dark" className="p-2">
              <CDBIcon fab icon="reddit" />
            </CDBBtn>
            <CDBBtn flat color="dark" className="p-2 mx-3">
              <CDBIcon fab icon="tiktok" />
            </CDBBtn>
          </CDBBox>
        </CDBBox>
      </footer>
    </>
  );
};
export default MyFooter;
